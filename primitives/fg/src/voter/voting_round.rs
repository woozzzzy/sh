	...
	N: Copy + BlockNumberOps + ::std::fmt::Debug,
{
	...
	incoming: E::In,
	outgoings: Vec<Buffered<E::Out, Message<H, N>>>,
	...
	N: Copy + BlockNumberOps + ::std::fmt::Debug,
{
	...
	pub(super) fn new(
		...
		log::debug!(
			target: "gramps",
			"🧓 New VotingRound for round {:?} with voting: {:?} and ids: {:?}.",
			round_number,
			voting.clone(),
			round_data.voter_ids.clone(),
		);

		let mut buffered_outs = Vec::new();
		for og in round_data.outgoings.into_iter() {
			buffered_outs.push(Buffered::new(og));
		}
	...
	pub(super) fn completed(
		...
		log::debug!(
			target: "gramps",
			"🧓 Completed VotingRound for round {:?} with voting: {:?} and ids: {:?}.",
			votes.number(),
			voting.clone(),
			round_data.voter_ids.clone(),
		);
	...
	fn primary_propose(&mut self, last_round_state: &RoundState<H, N>) -> Result<(), E::Error> {
		match self.state.take() {
			Some(State::Start(prevote_timer, precommit_timer)) => {
				...
				for (i, x) in self.voting.iter().enumerate() {
					if x.is_primary() {
						log::debug!(
							target: "gramps",
							"🧓 In primary_propose, voter at index {:?} is primary.",
							i,
						);
						if let Some(ref last_round_estimate) = maybe_estimate {
							...
							if should_send_primary {
								...
								self.outgoings[i].push(Message::PrimaryPropose(primary));
	...
	fn prevote(
		...
		let finish_prevoting = |this: &mut Self,
			...
			if let Some(target) = best_chain {
				...
				this.outgoings.iter_mut().for_each(|x| x.push(Message::Prevote(prevote.clone())));
				this.state = Some(State::Prevoted(precommit_timer));
			} else {
				...
				if should_precommit {
					for (i, x) in self.voting.iter().enumerate() {
						if x.is_active() {
							...
							self.outgoings[i].push(Message::Precommit(precommit));
						}
					};
